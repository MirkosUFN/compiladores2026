...
class PilhaCaracteres:
    """Pilha LIFO usada como fita de entrada do reconhecedor."""

    def __init__(self, texto: str = ""):
        # A inversão faz o primeiro caractere do texto ficar no topo.
        self._itens: List[str] = list(reversed(texto))

    def empilhar(self, caractere: str) -> None:
        if len(caractere) != 1:
            raise ValueError("A pilha aceita exatamente um caractere por vez.")
        self._itens.append(caractere)
...
class ResultadoAnalise:
    """Representa o resultado da análise léxica de um termo."""
...
def carregar_afd(caminho_config: str) -> AFD:
    ...
    if len(linhas) < 4:
        raise ValueError("Arquivo AFD_config.txt inválido ou incompleto.")
    ...
    if ESTADO_ERRO not in estados_definidos:
        raise ValueError(
            f"O estado de erro '{ESTADO_ERRO}' precisa estar declarado no AFD_config.txt."
        )
...
def executar_afd(
    ...
    """
    Executa o AFD desempilhando e reconhecendo um caractere por vez.
    ...
    pilha_entrada = PilhaCaracteres(lexema)

    if mostrar_transicoes:
        print(f"\n  Estado inicial: {estado_atual}")

    while not pilha_entrada.vazia():
        caractere = pilha_entrada.desempilhar()

        if estado_atual == ESTADO_ERRO:
            if mostrar_transicoes:
                print(
                    f"  desempilha '{caractere}' | "
                    f"{ESTADO_ERRO} --{caractere}--> {ESTADO_ERRO} | "
                    f"restantes: {len(pilha_entrada)}"
                )
            continue
        ...
        if caractere not in afd.simbolos:
            ...
            if mostrar_transicoes:
                print(
                    f"  desempilha '{caractere}' | "
                    f"{estado_atual} --{caractere}--> {ESTADO_ERRO} "
                    f"[ERRO] | restantes: {len(pilha_entrada)}"
                )
        ...
        if mostrar_transicoes:
            ...
            print(
                f"  desempilha '{caractere}' | "
                f"{estado_atual} --{caractere}--> {proximo_estado}{sufixo} | "
                f"restantes: {len(pilha_entrada)}"
            )
...
def criar_tabela_simbolos(caminho_csv: str):
    """
    Cria a tabela de símbolos com as colunas ID, TOKEN, TIPO e LINHA.
    """
    with open(
        caminho_csv,
        "w",
