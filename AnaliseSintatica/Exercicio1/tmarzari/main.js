...
const identificador = "[a-z_][a-z\\d_]*";
for (const [tipo, valor] of Object.entries(valoresPorTipo)) {
  const declaracao = valor && new RegExp(
    `^\\s*${tipo}\\s+${identificador}\\s*=\\s*${valor}(?:\\s*,\\s*${identificador}\\s*=\\s*${valor})*\\s*;\\s*$`
  );
  codigoFonte.forEach((linha, indice) => {
    if (new RegExp(`^\\s*${tipo}\\b`).test(linha) && linha.includes("=") && !declaracao?.test(linha)) {
      console.error(`Linha ${indice + 1}: declaração inicializada inválida.`);
      process.exitCode = 1;
    }
  });
}
